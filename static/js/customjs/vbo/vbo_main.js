$(document).ready(function() {
    ...
    var generate_callouts = function(){
        ...
        $.ajax({
            ...
            success: function(result){
             if (result.status == 'success')   {
                // alert('callouts successful')
                
                callouts = result.results
                console.log('callouts results == ', result.results)
                callouts.forEach(function(obj){
                    console.log('obj = ', JSON.parse(obj.results[0].report_data))
                })
                // x1 = result.results
             }
            }
    ...
    var show_weekly_report_17 = function(){
        ...
        click: function(event, ui){            
            ...
            if (ui.checked == true){
                $('#points-table tr:last').after('<tr id=' + id + '>'
                                ...
                                + '<td id="x-axis">' + contents[0].trim() + '</td> '
                                + '<td id="y-axis">' + contents[1].trim() + '</td>'  
                                + '<td id="point">' + contents[2].trim() + '</td>'  
                                + '<td id="callout">   <textarea/>  </td>'
                                + '<td id="x-axis-position">   <input> </td>'
                                + '<td id="y-axis-position">   <input> </td>'
                                + '<td id="box-type">   <select> <option>Box</option>   <option>Line</option> </select></td>'
                                + '<td id="angle">   <input id="angle"/></td>'
                                + '<td id="height">   <input id="height"/></td>'
                                + '<td id="width">   <input id="width"/></td>'
                                + '<td id="calloutbox-color">   <input class="jscolor" value="ab2567"/>    </td></tr>');
        ...
        $('#gen-graph').on('click', function() {        
            ...
            gen_points = {}
            generate_callout_points()
            $("#daily-report-2-highcharts").highcharts().redraw()
        ...
        var generate_callout_points = function(){
            ...
            $('#points-table tr').not(':first').each(function() {                
                ...
                x_axis = $(this).find("#x-axis").html();
                y_axis = $(this).find("#y-axis").html()
                point = $(this).find("#point").html()
                callout = $(this).find("#callout").find('textarea').val()
                y_axis_position = $(this).find("#x-axis-position").find('input').val()
                x_axis_position = $(this).find("#y-axis-position").find('input').val()
                angle = $(this).find("#angle").find('input').val()
                draw_type = $(this).find("#box-type").find('select').val()
                width = $(this).find("#width").find('input').val()
                height = $(this).find("#height").find('input').val()
             ...
             for (var key in gen_points) {
                 ...
                 gen_points[key].forEach(function(obj, index) {
                     ...
                     if (remove_labels) {
                         ...
                         if (draw_type == 'Box') {
                             // alert('draw')              
                             var a = chart.renderer.label('<div class="callout right" style="background-color:#' + gen_points[key][index]['callout_box_color'] + '">' + callout + '</div>',
